...
x=(sawtooth(w*t*10/22,0.5)+abs(sawtooth(w*t*10/22,0.5)))/2;


for n = -N:N

    C(n+N+1) = 1/T * integral(@(t)(1/2*sawtooth(w*t*10/22,0.5)+1/2*abs(sawtooth(w*t*10/22,0.5))).*exp(-1j*n*w*t*10/22),0,T) ;
...
for n = -N:N
    ...
    yr = yr + C(n+N+1)*exp(1j*n*w*t*10/22) ;
